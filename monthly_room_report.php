...
$title_report = '
<table><tr>
<td valign="bottom" align="center" width="100%"><h1>เดือน '.$month[$_GET['month']].' '.$_GET['year'].'</h1></td>
</tr></table>';
$pdf->writeHTML($title_report, true, false, true, false, '');

//-- สร้าง LOOP จำนวน ID ตึกทั้งหมด
$building = $db->getAllBuidling();
//echo "*****".count($building)."*****";
for ($idx = 0; $idx < count($building); $idx++){
	$buidingID = $building[$idx]['id'];



//-- หา Tb_room.id ในเดือนที่ต้องการ เพื่อเอาไปหาค่าใช้จ่ายอื่นๆ
$result = $db->showRentPerMonth($_GET['year'],$_GET['month'],$buidingID,'','');
...
$html = '<table>
		<tr>
			<td valign="bottom" align="left" width="30%"><h5>&nbsp;</h5></td>
			<td valign="bottom" align="left" width="15%"><h5>ห้อง</h5></td>
			<td valign="bottom" align="right" width="20%"><h5>ค่าห้องรวม</h5></td>
			<td valign="bottom" align="right" width="10%"><h5>&nbsp;</h5></td>
			<td valign="bottom" align="left" width="25%"><h5>&nbsp;</h5></td>
		</tr>';
...
while($rs = mysql_fetch_array($result)){
	...
	//-- ถ้าเดือนปัจจุบันเป็น มกราคม ให้ส่งค่าเดือนที่ต้องการค้นหาเป็น 13 เพราะเอาไปทำ < 
	if($_GET['month'] == "1"){
		$meter = $db->getMeterTwoLastMonthBTYDESC($rs['idRoom'],$_GET['year']);
	}else{
		$meter = $db->getMeterTwoLastMonthDESC($rs['idRoom'], $_GET['month'], $_GET['year']);
	}
	...
	$chkMonth = $db->checkMeterWithMonth($rs['idRoom'], $_GET['month'], $_GET['year']);
	...
	if(count($chkMonth)!=0){ //-- ถ้ายังไม่เก็บหน่วยเดือนนี้ ให้แสดงค่าว่างทั้งหมด
		if($output[0]['chargeperunitGov']==1){
			$total_elec = $cmonth['charge']; //--  ถ้าใช้ไฟฟ้ารัฐ ให้เอาราคาที่รัฐเก็บมาเป็นสุทธิเลย
		}else{
	...
	if(count($chkMonth)!=0){
		if($output[0]['waterperunitGov']==1){
			$total_wate = $cmonth['water']; //-- ค้าน้ำองค์การ ให้นำมาคำนวนสุทธิได้เลย
		}else{
			...
			//-- number_format($total_wate, 2)
		}
	}

	// ***************************************** //
	// รวม ค่าไฟฟ้า ค่าน้ำ ค่าอื่นๆ เพื่อหาค่าห้องสุทธิต่อเดือน //
	// ***************************************** //
	$total_other = $cmonth['etc'];	
	$total_price = $output[0]['rent'] + $total_elec + $total_wate + $total_other;	
	$total_month += $total_price;
	$html = $html.'
		<tr>
			<td valign="bottom" align="left" width="30%"><h5>&nbsp;</h5></td>
			<td valign="bottom" align="left" width="15%">'.$rs['roomName'].'</td>
			<td valign="bottom" align="right" width="20%">'.number_format($total_price,2).'</td>
...
$html = $html.'
		<tr>
			<td valign="bottom" align="left" width="30%"><h5>&nbsp;</h5></td>
			<td valign="bottom" align="left" width="15%"><h4>รวม</h4></td>
			<td valign="bottom" align="right" width="20%"><h4>'.number_format($total_month,2).'</h4></td>
			<td valign="bottom" align="right" width="10%"><h5>&nbsp;</h5></td>
			<td valign="bottom" align="left" width="25%"><h5>&nbsp;</h5></td>			
		</tr>
	</table>';
	/*
		<tr>
			...
			<td valign="bottom" align="left" width="35%"><h2>'.$rs['roomName'].'</h2></td>
